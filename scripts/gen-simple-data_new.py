...
all_data = RawDataOccurrence.objects.values('taibif_id', 'taibif_dataset_name','year', 'month', 'day','country', 'decimallongitude', 'decimallatitude', 'kingdom', 'phylum', 'class_field', 'order', 'family', 'genus', 'scientificname', 'specificepithet','vernacularname') \
    .filter(taibif_dataset_name='r10405002').order_by('taibif_id').all()
...
def proc_simple_data(data, sd):
    ...
    if total_c == 1:  # Genus+species
        ...
        try:  # Use scientific full name find
            ...
            try:
                taxon_data = Taxon.objects.values('id', 'name', 'rank', 'parent_id').filter(name=spname)
                if len(taxon_data) == 1:  # only one result in taxon
                    try:
                        # species_id, genus_id = taxa['sci_name'][spname]
                        sd.taxon_genus_id = taxon_data[0]['parent_id']  # from taxon get genus id
                        sd.taxon_species_id = taxon_data[0]['id']  # from taxon get species id
                        sd.spname = spname + '_one'
                    except:
                        pass
                elif len(taxon_data) > 1:  # not only one result in taxon
                    p_id = taxon_data[0]['parent_id']  # To check whether all the parent_id are same
                    count_id = 0
                    for dd in taxon_data:
                        if dd['parent_id'] == p_id:
                            count_id = count_id + 1
                    if count_id == len(taxon_data):  # Parent_id is same
                        try:
                            # species_id, genus_id = taxa['sci_name'][spname]
                            sd.taxon_genus_id = taxon_data[0]['parent_id']  # from taxon get genus id
                            sd.taxon_species_id = taxon_data[0]['id']  # from taxon get species id
                        ...
                        for data in taxon_data:
                            genus_test.append(data['parent_id'])
                        genus_list = Taxon.objects.values('id', 'name').filter(id__in=genus_test)

                        for gg in genus_list:

                            if gg['name'] == gename:
                                sd.taxon_genus_id = gg['id']  # from taxon get genus id
                                sd.taxon_species_id = data['id']  # from taxon get species id
    ...
    if (sd.taxon_family_id == None and sd.taxon_species_id != None):
        sd.taxon_family_id = \
            Taxon.objects.values('id', 'rank', 'name', 'parent_id').filter(rank='genus',id=sd.taxon_genus_id)[0]['parent_id']
        sd.taxon_order_id = \
            Taxon.objects.values('id', 'rank', 'name', 'parent_id').filter(rank='family',id=sd.taxon_family_id)[0]['parent_id']
        sd.taxon_class_id = \
            Taxon.objects.values('id', 'rank', 'name', 'parent_id').filter(rank='order',id=sd.taxon_order_id)[0]['parent_id']
        sd.taxon_phylum_id = \
            Taxon.objects.values('id', 'rank', 'name', 'parent_id').filter(rank='class', id=sd.taxon_class_id)[0]['parent_id']
        sd.taxon_kingdom_id = \
            Taxon.objects.values('id', 'rank', 'name', 'parent_id').filter(rank='phylum',id=sd.taxon_phylum_id)[0]['parent_id']


    else:  # Genus+species+others
