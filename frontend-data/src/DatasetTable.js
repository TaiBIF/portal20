...
class DatasetTable extends React.Component {
    ...
    applyFilters(filters) {
        const queryString = filters ? Array.from(filters).join("&") : "";
        const apiUrl =
            queryString === ""
                ? "/api/dataset/search"
    ...
    render() {
        ...
        if (error) {
            ...
            const dataList = data.results.map((v, i) => {
                ...
                const title =
                    v.status === "PUBLIC" ? (
                        <a href={"/dataset/" + v.taibif_dataset_id}>
                            {v.title}
                        </a>
                    ) : (
                        v.title
            ...
            for (let i = 1; i <= Math.ceil(data.count / data.limit); i++) {
                ...
                paginationList.push(
                    <li className={"page" + disabledPage} key={i}>
                        <a
                            style={{ margin: "10px" }}
                            href="#"
                            onClick={(e) =>
                                this.handlePaginate2(e, i, disabledPage)
                            }
                        >
                            {i}
                        </a>
            ...
            const theadItems = tableColName.map((v, i) => {
                ...
                return (
                    <th
                        ...
                        key={i}
                        onClick={(e) => this.handleSort(e, v[0])}
                    >
                        {v[1]}
                        {finger}
                    </th>
            ...
            return (
                <div>
                    ...
                    <nav aria-label="...">
                        <ul className="pager">{paginationList}</ul>
                    </nav>
                </div>
            );
        }
