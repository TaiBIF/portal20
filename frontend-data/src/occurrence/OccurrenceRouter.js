...
import OccurrenceGallery from "./OccurrenceGallery";
...
const OccurrenceRouter = ({ data, filters, urlPrefix, language }) => {
    ...
    const m = path.match(
        /\/occurrence\/(search|map|gallery|taxonomy|charts|download)/
    );
    ...
    for (let i in navTabsData) {
        ...
        if (!navTabsData[i].disable) {
            navTabs.push(
                <li
                    key={key}
                    className={activeTab === key ? "search-nav-active" : null}
                    onClick={(e) => setActiveTab(key)}
                >
                    <Link to={navTabsData[i].path + window.location.search}>
                        {language === "en"
                            ? navTabsData[i].key
                            : navTabsData[i].label}
                    </Link>
    ...
    return (
        <Router>
            <div className="table-responsive">
                <ul
                    className="nav nav-tabs nav-justified search-content-tab"
                    ref={navTabsRef}
                >
                    {navTabs.map((x) => x)}
                </ul>
