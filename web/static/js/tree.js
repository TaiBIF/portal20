'use strict';
{
  ...
  class SimpleTree extends Events {
    ...
    folder(node, parent = this.parent, before) {
      ...
      summary.dataset.taxon_id = node.taxon_id;
      summary.dataset.taxon_rank = node.rank;
      summary.dataset.taxon_name = node.name;
      details.appendChild(summary);
      this.append(details, parent, before, () => {
        details.open = node.open;
        details.dataset.type = SimpleTree.FOLDER;
        details.dataset.taxon_id = node.taxon_id;
        details.dataset.rank = node.rank;
      });
  ...
  class AsyncTree extends SimpleTree {
    ...
    folder(...args) {
      ...
      details.addEventListener('toggle', () => {
        ...
        // HACK, 亂改, details 找不到?, 所以無法 fetch?...
