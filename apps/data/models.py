...
class Dataset(models.Model):
    ...
    dwc_core_type = models.CharField(
        "Dw-C Core Type", max_length=128, choices=DWC_CORE_TYPE_CHOICE, null=True
    )
    ...
    has_publish_problem = models.BooleanField(
        "是否有發布問題 (IPT 裡黃色的區塊)",
        default=False,
        help_text="有可能 IPT 授權沒填?",
    )
    admin_memo = models.TextField(
        "後台管理註記", blank=True, null=True, help_text="不會在前台出現"
    )
...
class DatasetUpdateEvent(models.Model):
    dataset = models.ForeignKey(
        Dataset,
        on_delete=models.CASCADE,
        related_name="update_events",
    )
    dataset_mod_date = models.DateTimeField("資料集異動時間")
    captured_at = models.DateTimeField("快照建立時間", auto_now_add=True)
    source = models.TextField(blank=True, null=True)
    status = models.CharField(
        "狀態",
        max_length=10,
        choices=Dataset.STATUS_CHOICE,
        null=True,
        blank=True,
    )
    taibif_dataset_id = models.UUIDField()
    dataset_title = models.CharField(max_length=300, blank=True, null=True)
    dataset_name = models.CharField(max_length=300, blank=True, null=True)
    organization_name = models.TextField(blank=True, null=True)
    dwc_core_type = models.CharField(max_length=128, blank=True, null=True)
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["dataset_mod_date"], name="idx_ds_evt_mod_date"),
            models.Index(fields=["captured_at"], name="idx_ds_evt_captured"),
        ]
...
class Dataset_Contact(models.Model):
    ...
    position_type = models.CharField(max_length=128, null=True, blank=True)
...
class DatasetOrganization(models.Model):
    ...
    country_code = models.CharField(
        "國家代碼", max_length=8, default="TW", null=True, blank=True
    )
    administrative_contact = models.CharField(
        "行政聯絡人", max_length=256, default="", null=True, blank=True
    )
    endorsed_by = models.CharField(
        "發布單位認證節點", max_length=256, default="", null=True, blank=True
    )
    organization_gbif_uuid = models.CharField(
        "發布單位 GBIF UUID", max_length=256, default="", null=True
    )
    installations = models.CharField(
        "發布節點", max_length=256, default="", null=True, blank=True
    )
    technical_contact = models.CharField(
        "技術連絡人", max_length=256, default="", null=True, blank=True
    )
    country_or_area = models.CharField(
        "國家或區域", max_length=256, default="", null=True, blank=True
    )
    occurences_num = models.IntegerField(
        "出現紀錄筆數", default=0, null=True, help_text="(目前已不維護)", blank=True
    )
    dataset_num = models.IntegerField(
        "資料集數量", default=0, null=True, help_text="(目前已不維護)", blank=True
    )
...
class Taxon(models.Model):
    ...
    taicol_taxon_id = models.CharField(
        "taicol taxon id", max_length=128, null=True, blank=True
    )
    ...
    taicol_synonyms = models.CharField(
        "synonyms", max_length=1024, null=True, blank=True
    )
    formatted_synonyms = models.CharField(
        "formatted_synonyms", max_length=1024, null=True, blank=True
    )
    misapplied = models.CharField("misapplied", max_length=1024, null=True, blank=True)
    formatted_misapplied = models.CharField(
        "formatted_misapplied", max_length=1024, null=True, blank=True
    )
    ...
    alternative_name_c = models.CharField(
        "alternative_name_c", max_length=1024, null=True, blank=True
    )
    ...
    new_taxon_id = models.CharField(
        "new_taxon_id", max_length=32, null=True, blank=True
    )
    ...
    def synonyms(self):
        sys_name = (
            Taxon.objects.filter(accepted_name_id=self.id, is_accepted_name=False)
            .exclude(id=self.id)
            .exclude(taicol_name_id__isnull=True)
        )
    ...
    def taicol_search_link(self):
        url = "http://taibnet.sinica.edu.tw/chi/taibnet_species_list.php?T2={}&T2_new_value=true&fr=y".format(
            self.name
        )
        return url
        ...
        if self.path:
            ...
            for i in rank_list:
                try:
                    ...
                    print(
                        f"Taxon with taicol_taxon_id '{i}' does not exist in the database."
                    )
    ...
    def children(self):
        return Taxon.objects.filter(
            parent=self, is_accepted_name=True, is_in_taiwan=True
        ).all()
...
class Taxon_Picture(models.Model):
    ...
    taxon = models.ForeignKey(
        Taxon, on_delete=models.CASCADE, null=True, related_query_name="pic"
    )
    ...
    taicol_taxon_id = models.CharField(
        "taicol taxon id", max_length=1000, null=True, blank=True
    )


OCCURRENCE_COLUMN_MAP = {
    ...
    "taxonID": "taxon_id",
    "verbatimTaxonRank": "verbatim_taxon_rank",
    ...
    "georeferenceProtocol": "georeference_protocol",
    ...
    "verbatimCoordinateSystem": "verbatim_coordinate_system",
    "coordinatePrecision": "coordinate_precision",
    "locationAccordingTo": "location_according_to",
    ...
    "maximumElevationInMeters": "maximum_elevation_in_meters",
    "minimumDepthInMeters": "minimum_depth_in_meters",
    "maximumDepthInMeters": "maximum_depth_in_meters",
    ...
    "associatedReferences": "associated_references",
    ...
    "materialSampleID": "material_sample_id",
    "samplingProtocol": "sampling_protocol",
    "samplingEffort": "sampling_effort",
    "disposition": "disposition",
    "references": "references",
    "namePublishedInYear": "name_published_in_year",
...
class PublicDataManager(models.Manager):
    def get_queryset(self):
        public_dataset_names = [
            x["name"] for x in Dataset.public_objects.values("name").all()
        ]
        return (
            super().get_queryset().filter(taibif_dataset_name__in=public_dataset_names)
        )
    ...
    def filter_by_key_values(self, filters):
        ...
        for key, values in filters:
            ...
            if key == "q":
                ...
                #    if self.using == '':
                query = query.filter(
                    Q(vernacular_name__icontains=v) | Q(scientific_name__icontains=v)
                )
...
class Taxon20200619(models.Model):
    ...
    taxonid = models.TextField(
        db_column="taxonID", blank=True, null=True
    )  # Field name made lowercase.
    scientificnameid = models.TextField(
        db_column="scientificNameID", blank=True, null=True
    )  # Field name made lowercase.
    kingdom = models.TextField(blank=True, null=True)
    phylum = models.TextField(blank=True, null=True)
    class_field = models.TextField(
        db_column="class", blank=True, null=True
    )  # Field renamed because it was a Python reserved word.
    order = models.TextField(blank=True, null=True)
    family = models.TextField(blank=True, null=True)
    genus = models.TextField(blank=True, null=True)
    specificepithet = models.TextField(
        db_column="specificEpithet", blank=True, null=True
    )  # Field name made lowercase.
    infraspecificepithet = models.TextField(
        db_column="infraspecificEpithet", blank=True, null=True
    )  # Field name made lowercase.
    scientificnameauthorship = models.TextField(
        db_column="scientificNameAuthorship", blank=True, null=True
    )  # Field name made lowercase.
    vernacularname = models.TextField(
        db_column="vernacularName", blank=True, null=True
    )  # Field name made lowercase.
    taxonremarks = models.TextField(
        db_column="taxonRemarks", blank=True, null=True
    )  # Field name made lowercase.
    dataset_name = models.TextField(blank=True, null=True)
    scientificname = models.TextField(
        db_column="scientificName", blank=True, null=True
    )  # Field name made lowercase.
    taxonrank = models.TextField(
        db_column="taxonRank", blank=True, null=True
    )  # Field name made lowercase.
    taxonomicstatus = models.TextField(
        db_column="taxonomicStatus", blank=True, null=True
    )  # Field name made lowercase.
    acceptednameusageid = models.TextField(
        db_column="acceptedNameUsageID", blank=True, null=True
    )  # Field name made lowercase.
    subgenus = models.FloatField(blank=True, null=True)
    nameaccordingto = models.TextField(
        db_column="nameAccordingTo", blank=True, null=True
    )  # Field name made lowercase.
    nomenclaturalstatus = models.TextField(
        db_column="nomenclaturalStatus", blank=True, null=True
    )  # Field name made lowercase.
...
class Event20200619(models.Model):
    ...
    eventid = models.TextField(
        db_column="eventID", blank=True, null=True
    )  # Field name made lowercase.
    parenteventid = models.TextField(
        db_column="parentEventID", blank=True, null=True
    )  # Field name made lowercase.
    samplingprotocol = models.TextField(
        db_column="samplingProtocol", blank=True, null=True
    )  # Field name made lowercase.
    samplesizevalue = models.FloatField(
        db_column="sampleSizeValue", blank=True, null=True
    )  # Field name made lowercase.
    samplesizeunit = models.TextField(
        db_column="sampleSizeUnit", blank=True, null=True
    )  # Field name made lowercase.
    samplingeffort = models.TextField(
        db_column="samplingEffort", blank=True, null=True
    )  # Field name made lowercase.
    eventdate = models.TextField(
        db_column="eventDate", blank=True, null=True
    )  # Field name made lowercase.
    eventtime = models.TextField(
        db_column="eventTime", blank=True, null=True
    )  # Field name made lowercase.
    locationid = models.TextField(
        db_column="locationID", blank=True, null=True
    )  # Field name made lowercase.
    decimallatitude = models.TextField(
        db_column="decimalLatitude", blank=True, null=True
    )  # Field name made lowercase.
    decimallongitude = models.TextField(
        db_column="decimalLongitude", blank=True, null=True
    )  # Field name made lowercase.
    geodeticdatum = models.TextField(
        db_column="geodeticDatum", blank=True, null=True
    )  # Field name made lowercase.
    coordinateuncertaintyinmeters = models.FloatField(
        db_column="coordinateUncertaintyInMeters", blank=True, null=True
    )  # Field name made lowercase.
    coordinateprecision = models.FloatField(
        db_column="coordinatePrecision", blank=True, null=True
    )  # Field name made lowercase.
    dataset_name = models.TextField(blank=True, null=True)
    verbatimelevation = models.FloatField(
        db_column="verbatimElevation", blank=True, null=True
    )  # Field name made lowercase.
    verbatimlatitude = models.FloatField(
        db_column="verbatimLatitude", blank=True, null=True
    )  # Field name made lowercase.
    verbatimlongitude = models.FloatField(
        db_column="verbatimLongitude", blank=True, null=True
    )  # Field name made lowercase.
    verbatimcoordinatesystem = models.TextField(
        db_column="verbatimCoordinateSystem", blank=True, null=True
    )  # Field name made lowercase.
    year = models.FloatField(blank=True, null=True)
    month = models.FloatField(blank=True, null=True)
    day = models.FloatField(blank=True, null=True)
    habitat = models.TextField(blank=True, null=True)
    eventremarks = models.TextField(
        db_column="eventRemarks", blank=True, null=True
    )  # Field name made lowercase.
    countrycode = models.TextField(
        db_column="countryCode", blank=True, null=True
    )  # Field name made lowercase.
    county = models.TextField(blank=True, null=True)
    locality = models.TextField(blank=True, null=True)
    ...
    municipality = models.TextField(blank=True, null=True)
    geologicalcontextid = models.TextField(
        db_column="geologicalContextID", blank=True, null=True
    )  # Field name made lowercase.
    country = models.TextField(blank=True, null=True)
    stateprovince = models.TextField(
        db_column="stateProvince", blank=True, null=True
    )  # Field name made lowercase.
...
class CopyNew(models.Model):
    index = models.BigIntegerField(blank=True, null=True)
    occurrenceid = models.TextField(
        db_column="occurrenceID", blank=True, null=True
    )  # Field name made lowercase.
    occurrenceremarks = models.TextField(
        db_column="occurrenceRemarks", blank=True, null=True
    )  # Field name made lowercase.
    occurrencestatus = models.TextField(
        db_column="occurrenceStatus", blank=True, null=True
    )  # Field name made lowercase.
    institutionid = models.TextField(
        db_column="institutionID", blank=True, null=True
    )  # Field name made lowercase.
    institutioncode = models.TextField(
        db_column="institutionCode", blank=True, null=True
    )  # Field name made lowercase.
    ownerinstitutioncode = models.TextField(
        db_column="ownerInstitutionCode", blank=True, null=True
    )  # Field name made lowercase.
    collectionid = models.TextField(
        db_column="collectionID", blank=True, null=True
    )  # Field name made lowercase.
    collectioncode = models.TextField(
        db_column="collectionCode", blank=True, null=True
    )  # Field name made lowercase.
    catalognumber = models.TextField(
        db_column="catalogNumber", blank=True, null=True
    )  # Field name made lowercase.
    othercatalognumbers = models.TextField(
        db_column="otherCatalogNumbers", blank=True, null=True
    )  # Field name made lowercase.
    recordnumber = models.TextField(
        db_column="recordNumber", blank=True, null=True
    )  # Field name made lowercase.
    recordedby = models.TextField(
        db_column="recordedBy", blank=True, null=True
    )  # Field name made lowercase.
    fieldnumber = models.TextField(
        db_column="fieldNumber", blank=True, null=True
    )  # Field name made lowercase.
    fieldnotes = models.TextField(
        db_column="fieldNotes", blank=True, null=True
    )  # Field name made lowercase.
    basisofrecord = models.TextField(
        db_column="basisOfRecord", blank=True, null=True
    )  # Field name made lowercase.
    datasetid = models.TextField(
        db_column="datasetID", blank=True, null=True
    )  # Field name made lowercase.
    datasetname = models.TextField(
        db_column="datasetName", blank=True, null=True
    )  # Field name made lowercase.
    language = models.TextField(blank=True, null=True)
    type = models.TextField(blank=True, null=True)
    typestatus = models.TextField(
        db_column="typeStatus", blank=True, null=True
    )  # Field name made lowercase.
    coreid = models.TextField(blank=True, null=True)
    lifestage = models.TextField(
        db_column="lifeStage", blank=True, null=True
    )  # Field name made lowercase.
    eventtime = models.TextField(
        db_column="eventTime", blank=True, null=True
    )  # Field name made lowercase.
    ...
    month = models.TextField(blank=True, null=True)
    day = models.TextField(blank=True, null=True)
    startdayofyear = models.TextField(
        db_column="startDayOfYear", blank=True, null=True
    )  # Field name made lowercase.
    enddayofyear = models.TextField(
        db_column="endDayOfYear", blank=True, null=True
    )  # Field name made lowercase.
    eventdate = models.TextField(
        db_column="eventDate", blank=True, null=True
    )  # Field name made lowercase.
    ...
    verbatimeventdate = models.TextField(
        db_column="verbatimEventDate", blank=True, null=True
    )  # Field name made lowercase.
    verbatimdepth = models.TextField(
        db_column="verbatimDepth", blank=True, null=True
    )  # Field name made lowercase.
    kingdom = models.TextField(blank=True, null=True)
    phylum = models.TextField(blank=True, null=True)
    class_field = models.TextField(
    ...
    verbatimtaxonrank = models.TextField(
        db_column="verbatimTaxonRank", blank=True, null=True
    )  # Field name made lowercase.
    associatedtaxa = models.TextField(
        db_column="associatedTaxa", blank=True, null=True
    )  # Field name made lowercase.
    specificepithet = models.TextField(
        db_column="specificEpithet", blank=True, null=True
    )  # Field name made lowercase.
    ...
    acceptednameusage = models.TextField(
        db_column="acceptedNameUsage", blank=True, null=True
    )  # Field name made lowercase.
    acceptednameusageid = models.TextField(
        db_column="acceptedNameUsageID", blank=True, null=True
    )  # Field name made lowercase.
    originalnameusage = models.TextField(
        db_column="originalNameUsage", blank=True, null=True
    )  # Field name made lowercase.
    nameaccordingto = models.TextField(
        db_column="nameAccordingTo", blank=True, null=True
    )  # Field name made lowercase.
    higherclassification = models.TextField(
        db_column="higherClassification", blank=True, null=True
    )  # Field name made lowercase.
    taxonremarks = models.TextField(
        db_column="taxonRemarks", blank=True, null=True
    )  # Field name made lowercase.
    dateidentified = models.TextField(
        db_column="dateIdentified", blank=True, null=True
    )  # Field name made lowercase.
    identificationqualifier = models.TextField(
        db_column="identificationQualifier", blank=True, null=True
    )  # Field name made lowercase.
    identifiedby = models.TextField(
        db_column="identifiedBy", blank=True, null=True
    )  # Field name made lowercase.
    identificationverificationstatus = models.TextField(
        db_column="identificationVerificationStatus", blank=True, null=True
    )  # Field name made lowercase.
    previousidentifications = models.TextField(
        db_column="previousIdentifications", blank=True, null=True
    )  # Field name made lowercase.
    county = models.TextField(blank=True, null=True)
    country = models.TextField(blank=True, null=True)
    countrycode = models.TextField(
    ...
    highergeography = models.TextField(
        db_column="higherGeography", blank=True, null=True
    )  # Field name made lowercase.
    georeferenceddate = models.TextField(
        db_column="georeferencedDate", blank=True, null=True
    )  # Field name made lowercase.
    georeferencesources = models.TextField(
        db_column="georeferenceSources", blank=True, null=True
    )  # Field name made lowercase.
    georeferencedby = models.TextField(
        db_column="georeferencedBy", blank=True, null=True
    )  # Field name made lowercase.
    geodeticdatum = models.TextField(
        db_column="geodeticDatum", blank=True, null=True
    )  # Field name made lowercase.
    georeferenceprotocol = models.TextField(
        db_column="georeferenceProtocol", blank=True, null=True
    )  # Field name made lowercase.
    georeferenceremarks = models.TextField(
        db_column="georeferenceRemarks", blank=True, null=True
    )  # Field name made lowercase.
    georeferenceverificationstatus = models.TextField(
        db_column="georeferenceVerificationStatus", blank=True, null=True
    )  # Field name made lowercase.
    decimallongitude = models.TextField(
        db_column="decimalLongitude", blank=True, null=True
    )  # Field name made lowercase.
    ...
    verbatimlatitude = models.TextField(
        db_column="verbatimLatitude", blank=True, null=True
    )  # Field name made lowercase.
    verbatimlongitude = models.TextField(
        db_column="verbatimLongitude", blank=True, null=True
    )  # Field name made lowercase.
    verbatimlocality = models.TextField(
        db_column="verbatimLocality", blank=True, null=True
    )  # Field name made lowercase.
    verbatimcoordinates = models.TextField(
        db_column="verbatimCoordinates", blank=True, null=True
    )  # Field name made lowercase.
    ...
    locationaccordingto = models.TextField(
        db_column="locationAccordingTo", blank=True, null=True
    )  # Field name made lowercase.
    pointradiusspatialfit = models.TextField(
        db_column="pointRadiusSpatialFit", blank=True, null=True
    )  # Field name made lowercase.
    rights = models.TextField(blank=True, null=True)
    rightsholder = models.TextField(
        db_column="rightsHolder", blank=True, null=True
    )  # Field name made lowercase.
    license = models.TextField(blank=True, null=True)
    preparations = models.TextField(blank=True, null=True)
    id = models.TextField(primary_key=True)
    modified = models.TextField(blank=True, null=True)
    datageneralizations = models.TextField(
        db_column="dataGeneralizations", blank=True, null=True
    )  # Field name made lowercase.
    organismid = models.TextField(
        db_column="organismID", blank=True, null=True
    )  # Field name made lowercase.
    organismquantitytype = models.TextField(
        db_column="organismQuantityType", blank=True, null=True
    )  # Field name made lowercase.
    organismquantity = models.TextField(
        db_column="organismQuantity", blank=True, null=True
    )  # Field name made lowercase.
    sex = models.TextField(blank=True, null=True)
    individualcount = models.TextField(
        db_column="individualCount", blank=True, null=True
    )  # Field name made lowercase.
    verbatimelevation = models.TextField(
        db_column="verbatimElevation", blank=True, null=True
    )  # Field name made lowercase.
    minimumelevationinmeters = models.TextField(
        db_column="minimumElevationInMeters", blank=True, null=True
    )  # Field name made lowercase.
    maximumelevationinmeters = models.TextField(
        db_column="maximumElevationInMeters", blank=True, null=True
    )  # Field name made lowercase.
    minimumdepthinmeters = models.TextField(
        db_column="minimumDepthInMeters", blank=True, null=True
    )  # Field name made lowercase.
    maximumdepthinmeters = models.TextField(
        db_column="maximumDepthInMeters", blank=True, null=True
    )  # Field name made lowercase.
    waterbody = models.TextField(
        db_column="waterBody", blank=True, null=True
    )  # Field name made lowercase.
    island = models.TextField(blank=True, null=True)
    habitat = models.TextField(blank=True, null=True)
    reproductivecondition = models.TextField(
        db_column="reproductiveCondition", blank=True, null=True
    )  # Field name made lowercase.
    continent = models.TextField(blank=True, null=True)
    infraspecificepithet = models.TextField(
        db_column="infraspecificEpithet", blank=True, null=True
    )  # Field name made lowercase.
    footprintwkt = models.TextField(
        db_column="footprintWKT", blank=True, null=True
    )  # Field name made lowercase.
    associatedmedia = models.TextField(
        db_column="associatedMedia", blank=True, null=True
    )  # Field name made lowercase.
    associatedsequences = models.TextField(
        db_column="associatedSequences", blank=True, null=True
    )  # Field name made lowercase.
    associatedreferences = models.TextField(
        db_column="associatedReferences", blank=True, null=True
    )  # Field name made lowercase.
    nomenclaturalcode = models.TextField(
        db_column="nomenclaturalCode", blank=True, null=True
    )  # Field name made lowercase.
    footprintspatialfit = models.TextField(
        db_column="footprintSpatialFit", blank=True, null=True
    )  # Field name made lowercase.
    establishmentmeans = models.TextField(
        db_column="establishmentMeans", blank=True, null=True
    )  # Field name made lowercase.
    behavior = models.TextField(blank=True, null=True)
    informationwithheld = models.TextField(
        db_column="informationWithheld", blank=True, null=True
    )  # Field name made lowercase.
    islandgroup = models.TextField(
        db_column="islandGroup", blank=True, null=True
    )  # Field name made lowercase.
    municipality = models.TextField(blank=True, null=True)
    materialsampleid = models.TextField(
        db_column="materialSampleID", blank=True, null=True
    )  # Field name made lowercase.
    samplingprotocol = models.TextField(
        db_column="samplingProtocol", blank=True, null=True
    )  # Field name made lowercase.
    ...
    disposition = models.TextField(blank=True, null=True)
    references = models.TextField(blank=True, null=True)
    namepublishedinyear = models.TextField(
        db_column="namePublishedInYear", blank=True, null=True
    )  # Field name made lowercase.
    namepublishedin = models.TextField(
        db_column="namePublishedIn", blank=True, null=True
    )  # Field name made lowercase.
    dataset_name = models.TextField(blank=True, null=True)
    ...
    identificationremarks = models.TextField(
        db_column="identificationRemarks", blank=True, null=True
    )  # Field name made lowercase.
    parenteventid = models.TextField(
        db_column="parentEventID", blank=True, null=True
    )  # Field name made lowercase.
...
class WorkshopCertificationList(models.Model):
    LEVEL_CHOICES = (("basic", "核心"), ("advanced", "增能"))
...
class TaibifParticipants(models.Model):
    ...
    role = models.TextField(
        "角色類型", blank=False, help_text="（若有一個以上的項目，請用全形頓號分隔）"
    )
    missions = models.TextField(
        "參與任務", blank=False, help_text="（若有一個以上的項目，請用全形頓號分隔）"
    )
    last_update = models.DateTimeField("最後修改日期", auto_now_add=True)
...
class DataPaperList(models.Model):
    ...
    article_doi = models.CharField(
        "文章 doi",
        blank=False,
        help_text="（請填入 doi 即可，例：10.3897/BDJ.10.e90196。前端會自動轉換成連結）",
    )
    dataset_doi = models.CharField(
        "資料集 doi",
        blank=False,
        help_text="（請填入 doi 即可，例：10.15468/43z4mj。前端會自動轉換成連結）",
    )
    year = models.IntegerField(
        "發布年份", blank=False, help_text="（會依照發布年份顯示在前端，由新至舊排序）"
    )
    last_update = models.DateTimeField("最後修改日期", auto_now_add=True)
...
class Taibifer(models.Model):
    ...
    year = models.IntegerField(
        "認證年份", blank=False, help_text="（會依照發布年份顯示在前端，由新至舊排序）"
    )
    last_update = models.DateTimeField("最後修改日期", auto_now_add=True)
